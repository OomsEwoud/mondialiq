...
export default function PredictionCard({ match, actionLabel, mode }: Props) {
    ...
    return (
        <article className="group rounded-2xl border border-slate-200 bg-white/92 p-4 shadow-sm shadow-blue-950/5 backdrop-blur transition-all hover:-translate-y-0.5 hover:border-cyan-200 hover:shadow-lg hover:shadow-blue-950/10 sm:p-5">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="grid min-w-0 gap-4">
                    <span
                        className={
                            isMine
                                ? 'w-fit rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-black tracking-[0.16em] text-slate-700 uppercase'
                                : 'w-fit rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1 text-[11px] font-black tracking-[0.16em] text-cyan-800 uppercase'
                        }
                    >
